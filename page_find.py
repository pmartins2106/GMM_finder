...
def page_find():
    ...
    def fit_fingerprints(df):
        ...
        # kcat vs. [X]
        ydata0 = df.iloc[:,1]
        ig = np.asarray([1,max(ydata0),np.mean(xdata)])
            ...
            with st.spinner("Fitting... Please wait a moment."):
                ...
                if data_comp_chk:
                    ...
                    if final_polish:
                        ...
                        # p0 = k2_0, Km_0, Kx_0
                        p0 = 1, 1, np.mean(X_exp)
                        # y1 = fun_eq_global((S,X),1,1,10,10,10)
                        # st.write(XDATA[0:NS], XDATA[NS:NS+NX])
                        # XDATA=np.column_stack([S,X])
                        
                        popt, pcov = curve_fit(fun_eq_global,XDATA, y, p0)
                        alpha = popt[0]
                        beta= popt[1]
                        Kx = popt[2]
                                               
                        results_fgrprt = pd.DataFrame([ [alpha, beta, k2_0, Km_0, Kx ]],
                                                      index = ['Values'],
                                                      columns = ['alpha','beta','k2','Km', 'Kx'])
                        st.dataframe( results_fgrprt)
                        
                        # selector(p_alpha, p_beta, p_Km, p_Kx)                      
                        Mechanism = selector(alpha, beta, Km_0, Kx )
