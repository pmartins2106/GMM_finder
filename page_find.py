...
def page_find():
    ...
    def fit_fingerprints(df):
        ...
        # Kmvs. [X]
        ydata1 = df.iloc[:,3]
        ig = np.asarray([1,max(ydata1),np.mean(xdata)])
        ...
        fit_y = fun_kcat(x_fit, beta, k2, Kca)
        axs[0].plot(x_fit, fit_y)
        axs[0].set(ylabel = 'kcat_app')
          
        axs[1].scatter(xdata, ydata1)
        fit_y = fun_Km(x_fit, alpha, Km, Kx)
        axs[1].plot(x_fit, fit_y)
        plt.xlabel('[X]')
        plt.ylabel('Km_app')
        
        return alpha, beta, k2, Km, Kx, fig_fgpts
    
    
    # Global fit
    @st.cache_data()
    def fun_eq_global(XDATA,k2,Km,Kx):
        """
        ...
        if data_fit_chk:
            with st.spinner("Fitting... Please wait a moment."):
                ...
                if data_comp_chk:
                    ...
                    alpha_0, beta_0, k2_0, Km_0, Kx_0, fig_fgpts = fit_fingerprints(results)
                    st.pyplot(fig_fgpts)
                    
                    st.info('First estimates of parameters alpha, beta, k2, Km and Kx')
                    results_fgrprt_0 = pd.DataFrame([ [alpha_0, beta_0, k2_0, Km_0, Kx_0 ] ],
                                                  index = ['Values'],
                    ...
                    if final_polish:
                        ...
                        y = y.flatten()
                        # y = np.array(y)
                        S = df.iloc[:,0].astype(float) #.to_numpy()
                        X = results.iloc[:, 0].astype(float)
                        
                       
                        # initial guesses for k2, Km, Kx
                        p0 = k2_0, Km_0, Kx_0
                        
                        # y1 = fun_eq_global((S,X),1,1,10,10,10)
                        # st.write(y1.shape,y.shape)
                        popt, pcov = curve_fit(fun_eq_global,(S,X), y, p0)
                        k2 = popt[0]
                        Km = popt[1]
                        Kx = popt[2]
                                               
                        results_fgrprt = pd.DataFrame([ [alpha_0, beta_0, k2, Km, Kx ]],
                                                      index = ['Values'],
                                                      columns = ['alpha','beta','k2','Km', 'Kx'])
                        st.dataframe( results_fgrprt)
                        
                        # selector(p_alpha, p_beta, p_Km, p_Kx)                      
                        Mechanism = selector(alpha_0, beta_0, Km, Kx )
